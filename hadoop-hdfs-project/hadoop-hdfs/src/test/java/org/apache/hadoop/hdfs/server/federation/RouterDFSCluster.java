...
public class RouterDFSCluster {
  ...
  public class RouterContext {
    ...
    public void initRouter() throws URISyntaxException {
      ...
      if (rpcAddress != null) {
        ...
        this.fileSystemUri =
            new URI("hdfs://" + NetUtils.getHostPortString(rpcAddress));
  ...
  public class NamenodeContext {
    ...
    public void setNamenode(NameNode n) throws URISyntaxException {
      ...
      // RPC
      rpcPort = n.getServiceRpcAddress().getPort();
    ...
    public String getRpcAddress() {
      return namenode.getServiceRpcAddress().getHostName() + ":" + rpcPort;
    }
  ...
  private int cacheFlushInterval;
  ...
  public Configuration generateRouterConfiguration(String localNameserviceId,
    ...
    conf.setInt(DFSConfigKeys.DFS_ROUTER_HANDLER_COUNT_KEY, 10);
    ...
    conf.set(DFSConfigKeys.DFS_ROUTER_DEFAULT_NAMESERVICE, nameservices.get(0));
    conf.setInt(DFSConfigKeys.DFS_ROUTER_HEARTBEAT_INTERVAL_SECS_KEY,
        heartbeatInterval);
    conf.setInt(DFSConfigKeys.FEDERATION_ROUTER_CACHE_TIME_TO_LIVE_SECS_KEY,
        cacheFlushInterval);
    ...
    conf.setInt(DFSConfigKeys.DFS_ROUTER_SAFEMODE_STARTUP_INTERVAL, 1);
    ...
    conf.set(DFSConfigKeys.DFS_ROUTER_MONITOR_NAMENODE,
        routerBuilder.toString());
    if (this.routerOverrides != null) {
      for (Iterator<Entry<String, String>> iter =
          routerOverrides.iterator(); iter.hasNext();) {
        Entry<String, String> element = iter.next();
        // Add custom overrides
        conf.set(element.getKey(), element.getValue());
      }
    }
    return conf;
  }
    ...
    try {
      ...
      for (String ns : nameservices) {
        ...
        if (highAvailability) {
          ...
          conf.addNN(new MiniDFSNNTopology.NNConf(null));
        }
  ...
  public void waitActive(RouterContext router)
    ...
    for (int loopCount = 0; loopCount < 20; loopCount++) {
      ...
      if (router.router.getRouterState() == RouterServiceState.RUNNING) {
        return;
      }
      Thread.sleep(1000);
    }
    ...
    for (RouterContext r : routers) {
      ...
      for (NamenodeContext nn : namenodes) {
        ...
        NamenodeStatusReport report = new NamenodeStatusReport(nn.nameserviceId,
            nn.namenodeId, nn.getRpcAddress(), nn.getHttpAddress());
  ...
  public void switchToActive(String nameservice, String namenode) {
    try {
      int total = cluster.getNumNameNodes();
      NameNodeInfo namenodes[] = cluster.getNameNodeInfos();
      for (int i = 0; i < total; i++) {
        NameNodeInfo n = namenodes[i];
        if (n.getNameserviceId().equals(nameservice)
            && n.getNamenodeId().equals(namenode)) {
  ...
  public void switchToStandby(String nameservice, String namenode) {
  ...
  public void stopNamenode(String nameservice, String namenode) {
    int total = cluster.getNumNameNodes();
    NameNodeInfo namenodes[] = cluster.getNameNodeInfos();
    for (int i = 0; i < total; i++) {
      NameNodeInfo n = namenodes[i];
  ...
  public void stopRouter(RouterContext router) {
    try {
      ...
      int loopCount = 0;
      while (router.router.getRouterState() != RouterServiceState.SHUTDOWN) {
        loopCount++;
        Thread.sleep(1000);
        if (loopCount > 20) {
