...
public class MockResolver
  ...
  public MockResolver(Configuration conf, FederationStateStoreService store) {
  ...
  public List<? extends FederationNamenodeContext>
      getPrioritizedNamenodesForNameserviceId(String nameserviceId) {
  ...
  private static class MockNamenodeContext
    ...
    public MockNamenodeContext(String rpc, String web, String ns, String nn,
  ...
  public synchronized boolean registerNamenode(NamenodeStatusReport report)
    ...
    MockNamenodeContext context = new MockNamenodeContext(
        report.getServiceAddress(), report.getWebAddress(),
        report.getNameserviceId(), report.getNamenodeId(), report.getState());

    @SuppressWarnings("unchecked")
    List<MockNamenodeContext> existingItems =
        (List<MockNamenodeContext>) resolver
            .get(report.getNameserviceId());
    if (existingItems == null) {
      existingItems = new ArrayList<MockNamenodeContext>();
      resolver.put(report.getBlockPoolId(), existingItems);
      resolver.put(report.getNameserviceId(), existingItems);
    }
    existingItems.add(context);
    Collections.sort(existingItems, new NamenodePriorityComparator());

    FederationNamespaceInfo info =
        new FederationNamespaceInfo(report.getBlockPoolId(),
            report.getClusterId(), report.getNameserviceId());
    namespaces.add(info);
    return true;
  }
