...
(function () {
  ...
  dust.loadSource(dust.compile($('#tmpl-rebalancer').html(), 'rebalancer'));
  ...
  function load_rebalancer() {
    ...
    function workaround(r) {
      ...
      r.Rebalancer = node_map_to_array(JSON.parse(r.RebalancerHistory));
      return r;
    }
    ...
    $.get(
      ...
      guard_with_startup_progress(function (resp) {
        var data = workaround(resp.beans[0]);
        var base = dust.makeBase(HELPERS);
        dust.render('rebalancer', base.push(data), function(err, out) {
          $('#tab-rebalancer').html(out);
          $('#ui-tabs a[href="#tab-rebalancer"]').tab('show');
        });
      })).error(ajax_error_handler);
  }
