...
public interface RouterRpcMonitor {
  ...
  void initialize(Configuration conf, RouterRpcServer server);
  ...
  /**
   * Track accesses to a path in the federated name space.
   * @param path Path to log.
   */
  void logPathStat(String path);

  /**
   * Get the access statistics for the namespace.
   * @return Access statistics to each path.
   */
  PathTree<Long> getPathStats();
}
